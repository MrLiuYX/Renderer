...
namespace Native
{
    public class ReferenceCollection
    {
        ...
        public void Release(IReference reference)
        {
            ...
            {
                Debug.LogWarning($"[mmmyyliu] Can't release reference {reference.GetType().Name}");
                return;
            }
        ...
        public void InspectorDraw(string search)
        {
            if(!string.IsNullOrEmpty(search))
            {
                if(!_type.FullName.ToLower().Contains(search.ToLower()))
                {
                    return;
                }
            }
