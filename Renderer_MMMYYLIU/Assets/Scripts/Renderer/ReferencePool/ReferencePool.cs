...
namespace Native
{
    public static class ReferencePool
    {
        ...
        public static object Acquire(Type t)
        {
            if(!typeof(IReference).IsAssignableFrom(t))
            {
                //Debug.LogWarning($"Can't coverts type from {t} to IReference");
                return null;
            }

            var temp = _referenceCollections.Find(x => x.CheckType(t));
            if (temp == null)
            {
                temp = new ReferenceCollection(t);
                _referenceCollections.Add(temp);
            }
            return temp.Acquire();
        }
        ...
        private static string _debugTxt;
        public static void InspectorDraw()
        {
#if UNITY_EDITOR
            _debugTxt = GUILayout.TextField(_debugTxt);
            for (int i = 0; i < _referenceCollections.Count; i++)
            {
                _referenceCollections[i].InspectorDraw(_debugTxt);
            }
#endif
        }
    }
